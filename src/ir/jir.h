...
typedef enum {
    ...
    /* SSA */
    JIR_PHI,
    /* BEAC: absolute address of the global data area. The non-local access
     * machinery is built on this -- program-level variables, and later the
     * display itself, live in global storage the backend reaches by address
     * rather than off the stack. */
    JIR_GADDR,
    JIR_OP_COUNT
} jir_op_t;
