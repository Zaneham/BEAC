                    ...
                    uint32_t cur_blk)
{
    ...
    /* ---- Global data base (BEAC) ---- */

    case JIR_GADDR:
        /* the global area lives at a fixed runtime address, baked straight
         * in; GEP then walks to the field, no frame pointer in sight. */
        e_mov_ri(X, R_RAX, X->globals_base);
        st_val(X, ii, R_RAX);
        break;
    ...
    case JIR_CALL: {
        ...
        /* store result -- a float-typed call hands it back in XMM0, an
         * integer one in RAX (Win64). */
        if (is_flt(X, I->type))
            st_fval(X, ii, R_XMM0);
        else
            st_val(X, ii, R_RAX);
